...
class Sequencer():
    ...
    def init_page_1_values(self):
        self.sequencer_note = 36
        self.update_colors()

    def update_colors(self):
        self.note_off_color = self.sequencer_note - 36
        self.note_on_color = self.sequencer_note + 28
        self.note_follow_color = self.sequencer_note + 60

    def init_buttons_page_1(self):
        # Create encode buttons
        self.init_page_1_values()
        ...
        if actual_clip and actual_clip.is_midi_clip:
            ...
            if value > 0:
                ...
                if not self.pad_status[cc_value]:
                    self._send_midi((177, sender._msg_identifier,
                                     self.note_on_color))
                    actual_clip.set_notes(((self.sequencer_note, cc_value*0.25,
                                            0.25, 127, False)))
                    self.pad_status[cc_value] = True
                else:
                    self._send_midi((177, sender._msg_identifier,
                                     self.note_off_color))
                    actual_clip.remove_notes(cc_value*0.25,
                                             self.sequencer_note, 0.25, 1)
                    self.pad_status[cc_value] = False
